...
class Scalar0Server(BaseServer):
    ...
    async def event_on_login_completed(self, client: Scalar0Client):
        self._userlist.append(client._user)
        self.broadcast(protocol.CLIENTBOUND_EventUserJoined(client._user))
    ...
    async def event_on_socket_broken(self, client: Scalar0Client):
        ...
        self.broadcast(protocol.CLIENTBOUND_EventUserLeft(client._user))
    ...
    async def _process_packet(self, client: Scalar0Client, packet_type: type, packet: protocol.packet.Packet):
        ...
        if packet_type is protocol.SERVERBOUND_SendMessage:
            ...
            return await self.broadcast(protocol.CLIENTBOUND_UserMessage(fingerprint=client._user.fingerprint, message=packet.message))
